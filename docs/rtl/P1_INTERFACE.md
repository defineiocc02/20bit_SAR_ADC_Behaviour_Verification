...
- 分布符合 `round(uniform(-D,D))`（端点概率为内点一半）、均值 ≈ 0、跨拍不相关；
- 边界值（`−D`、`+D`、`0`）必须真的出现过。
...
**验收判据（ADR 0016）**：支撑集精确；D>0 时端点概率 `1/(4D)`、内点 `1/(2D)`；
均值 ≈ 0；一阶相关系数绝对值 <0.03。T6 和 `review_dither_tb` 都在 SV 中直接断言，
先排除启动瞬态；后者覆盖 D=0/1/2/3/63、三个非零种子、拒采样、保持与复位。
这冻结了统计分布，不要求与 Python PCG64 逐位一致。
...
| T6 M4 `dither_gen` | 0 错误；200000 拍支撑集统计（历史门禁未检查 PMF；见 ADR 0016 新回归），valid rate 1.0000 |
