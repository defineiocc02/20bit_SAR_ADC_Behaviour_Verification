...
% 版本基线：v7.0.9 (commit 35dab64)；主链路参考数据 results.json 指纹
% a1ccd92f…35ac70（v7.0.5 锁定，不随本版变化）；M11–M13 为 v7.0.8 新增
% 模块测试，不在该指纹内。
% ---------------------------------------------------------------------------
...
\author{adi\_model 行为级验证模型（v7.0.9）}
...
（\texttt{adi\_model} v7.0.9）的仿真输出逐条对照，给出定量与定性复现
...
\begin{itemize}[itemsep=1pt]
  \item \textbf{三层证据元数据（不得混写）}：(i)~当前代码与报告版本
        = v7.0.9（commit \texttt{35dab64}）；(ii)~主链路参考数据 =
        \texttt{tools/results/results.json}（SHA256~$=$~
        \texttt{a1ccd92fcf46500460221a55c4e2527093b45e6e272b7fb0503
        ee1fcd435ac70}，v7.0.5 发版时全量复跑逐字节比对）；(iii)~新增
        机制验证 M11--M13 = 模块测试入口（v7.0.8 起），\textbf{不在}
        该 JSON 指纹覆盖范围内。
  ...
  \item 参数来源分级由 \texttt{provenance.py} 在运行时强制执行：
        ...
        本报告所有证据行均附等级标签。
  \item 工程状态：v7.0.9（commit \texttt{35dab64}），门禁 ruff /
        format / mypy 全净，pytest \textbf{223 passed, 4 xfailed}，
        远端 CI 结论 success。
\end{itemize}
...
M10 & 失配预算 $\to$ 校准必要性（架构论证） & $\sigma=300$\,ppm 是本工程假设目标 \GRADE{ASSUMED}（原文仅披露 ``$>$12b AC matching''，未给单位电容 $\sigma$ 指标）：该目标下均值 92.56\,dB 未达 93\,dB；模型均值口径达标上限 $\approx$200\,ppm；PDK 估算 1117\,ppm，缺口 3.7$\times$；MC 16 片：93.30--93.72\,dB（FITTED） & \rec{budget}、\rec{mc}、\rec{derived}\\
M11 & [12] 交织跟踪相位电荷核算 & $v_{\mathrm{hold}}[i]=x[i{-}1]$ 逐拍恒等式（最近另一颗结果，仅差 1 个复合周期）；慢信号 kickback 电荷比 reset $<0.2$；近 Nyquist 区间如实复现。带宽/噪声收益不按电荷比例换算（相对约束与电荷无关；绝对约束下电荷进对数项） & 模块测试（v7.0.8 新增，不在 results.json 指纹内）\\
M12 & [13] 辅助输入建立标度律 & $R_f$ 上限放大 $(C_f{+}C_{pg})/C_f$ 倍、最低带宽下界同比例降、噪声 rms $\propto\sqrt{\mathrm{BW}}$；辅助通路建立可行性已检查（$\tau_{aux}$ 超窗即拒绝）；gate\_boost 零调制 $=$ 理想自举对照 & 同上\\
M13 & [14] 低功耗参考阈值逼零回路（A06 机制层） & 整定周期数由 [假设] 增益决定（$g{=}0.9\to$6、$0.6\to$12、$0.3\to$25）——``5 or 6''量级一致非独立验证；外部补电荷衰减 $>2$ 个量级；粗判决误差在主配置契约窗（4.6875\,mV）内、末位试 $>$20b 等效。RA/ADC2 联合建立未验证 & 同上\\
\end{longtable}
\endgroup
...
\textbf{两个口径不得混写}：$\sigma=300$\,ppm 是\textbf{本工程
假设的失配目标}（\GRADE{ASSUMED}）——[00\_1]~p.9 只披露
``$>$12b AC matching (SADC to RDAC)''的采样响应匹配要求，未给单位
电容随机失配 $\sigma$ 指标；300\,ppm 不是模型的达标点，也不是
文献直接披露值。模型自身的 $\sigma$ 扫描
（\rec{budget.rows}）给出：SNDR 均值 $\geq$~93\,dB 的达标上限
$\approx$~200\,ppm（93.13\,dB@200；92.56\,dB@300 已低于目标），最小值
口径 $\approx$~150\,ppm。PDK 估算 1117\,ppm 相对假设目标 3.7$\times$
...
\caption{\textbf{失配预算与校准动机（M10/Q4）}——左：论证示意
（自绘）：\textbf{本工程假设目标} 300\,ppm \GRADE{ASSUMED} vs PDK 估算 1117\,ppm（3.7$\times$），
...
$\approx$200\,ppm（与假设目标 300\,ppm 是两个口径）；下：MC 16 片
（横轴为芯片编号，全部芯片同一 $\sigma=100$\,ppm 配置）
...
慢信号下 kickback 电荷比 reset \textbf{$<0.2$}。\textbf{带宽/噪声
收益不按电荷比例换算}（第八份外部复核订正）：相对建立约束下
$f_{\min}=\ln(1/\varepsilon)/(2\pi T)$ 与电荷无关；绝对残差约束下
$f_{\min}=\max\bigl(0,\ln\frac{q}{C_f v_{\mathrm{err}}}\bigr)/(2\pi T)$，
电荷只进对数项；噪声 rms $\propto\sqrt{\mathrm{BW}}$。旧版
``$f\propto q$''公式量纲不成立（$q/(T\,a_{tol})$ 的量纲是 F/s 而非
Hz），已撤回——电荷减少是收益的必要条件，不是换算系数。加权模式
复现披露例（N=3，10/30/60\%）。近 Nyquist 区间（reset 反而更优，
[12] 背景段 $Q{=}2A$ 算术的均值版）\textbf{如实复现}，不做选择性
呈现。机制级证据：模块测试 19 项（不在 \texttt{results.json} 锁定
指纹内）；主链路集成未做（N2）。
...
$R_f$ 允许放大 $(C_f{+}C_{pg})/C_f$ 倍，最低要求带宽下界同比例
下降，驱动噪声 rms $\propto\sqrt{\mathrm{BW}}\propto 1/\sqrt{R_f}$
（第八份复核订正方向）；原文量级锚点（p.12）：50\,kHz 低频信号也
要求 \textbf{77\,MHz} 滤波带宽——低频输入仍可能要求很宽前级。
\textbf{收益前提已入模型}：辅助通路自身须在采集窗内建立（
$\tau_{aux}=R_{aux}C_{pg}\le t_{acq}/\ln(1/\varepsilon_{aux})$），
否则 \texttt{required\_filter\_bw} 拒绝报告收益。gate\_boost 的
$r_{on}$ 零调制是\textbf{理想自举对照}（仅关 $V_{GS}$ 项，忽略阈值/
体效应/有限 $V_{DS}$/自举建立误差），非器件结论。主链路无第二输入
端口（N2）。机制级证据：模块测试 17 项。

\paragraph{M13（[14] 低功耗参考）：A06 机制层的回答。}
[14]~US~10,826,519~B1 以慢速功放 + 常充电大电容保持内部参考，转换
前只需极少量外部补电荷；模型（\texttt{ref\_track.py}）以限流充电 +
一阶伺服 + S1 泄放的行为级电荷-阈值回路。\textbf{整定周期数由
[假设] 增益 $g$ 决定}（复核实测：$g{=}0.9\to$6、$0.6\to$12、
$0.3\to$25，从零计数）——$g{=}0.9$ 参数组的 6 周期与披露
``5 or 6''量级一致，\textbf{但不构成对披露整定行为的独立验证}。
外部补电荷随周期衰减 $>2$ 个量级；阈值不动点 $\theta^{*}=-r$（跟随
滞后，相对逼零）。对审计 A06 的回答落在\textbf{机制层}：粗判决位试
的参考误差 RMS $\approx$2.0\,mV，落在主配置残差量程契约窗内（ADC2
余量 $0.15$\,V$/G_0{=}32=4.6875$\,mV，\textbf{非} $\Delta_1/2$
口径——自愈窗由残差实际范围与 RA/ADC2 限制共同决定），末位试误差
$>$20b 等效——``参考仅 ~15b 精度时 RA 起动、尾段才到 20b''的机制
结构自洽。\textbf{边界}：本模块没有 RA 状态、RA 有限带宽与 ADC2
采样窗口，``动态参考—RA—ADC2 联合建立''仍未验证；\textbf{末位试
参考误差很小 $\neq$ RA 对整个参考扰动的积分/跟踪误差很小}。主链路
参考路径仍为瞬时增益（N2）。
...
{}[13] US 10,541,702 B1 & 辅助输入抵消输入电荷 & \texttt{aux\_input.py}：$R_f$ 上限放大 $(C_f{+}C_{pg})/C_f$ 倍、噪声 $\propto 1/\sqrt{R_f}$、可行性检查、理想自举对照 & \textcolor{warnamber}{机制级实现，主链路未接入}\\
{}[14] US 10,826,519 B1 & 低功耗参考 & \texttt{ref\_track.py}：独立行为模型（整定随 [假设] 增益 $g$ 而定，$g{=}0.9\to$6）；粗判决容差 $=$ 主配置契约 4.6875\,mV；RA/ADC2 联合建立未验证 & \textcolor{warnamber}{机制级实现，主链路未接入}\\
\bottomrule
\end{tabular}
...
\begin{enumerate}[itemsep=1pt]
  ...
  \item 测试：\texttt{pytest}（223 passed, 4 xfailed）；沙箱环境如遇
...
\toprule
代码/报告版本 & v7.0.9（commit \texttt{35dab64}，tag 同步）\\
主链路参考数据 & results.json 指纹 \texttt{a1ccd92f…35ac70}（v7.0.5 锁定）\\
新增机制验证 & M11--M13 模块测试入口（v7.0.8 起，不在该指纹内）\\
门禁 & ruff / ruff-format / mypy 全净；pytest 223 passed, 4 xfailed\\
