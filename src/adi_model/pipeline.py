...
============================================================================
  * τ 口径 = **聚合单节点 RC**（τ0=(R_s+R_on)·C_load，与 sim_split/stage13
    同一口径）。"真实架构 τ_i = R·C_slice 小 ~8 倍、本口径偏保守"这个说法
    **只对支路开关项成立**：公共源阻抗 R_s 后挂 N 条 (R_on+C_slice) 支路的
    星形网络里，公共模式 τ = R_s·C_total + R_on·C_slice（第五份复核 §5.2），
    即 R_s·C_load 项**不随 slice 划分缩小**。示例（N=8, C_total=20.5 pF,
    R_s=30 Ω, R_on=20 Ω）：聚合式 1.025 ns，星形公共模式 0.666 ns（仅
    1.54×，不是 8×），全部按单 slice 算 0.128 ns。结论：R_s 主导时两种口径
    趋同，"偏保守"随之失效；指导输入驱动设计至少要保留公共节点与支路节点
    两层，不能只用一个等效 RC。slice 间带宽差异由 cfg.slice_bw_spread 刻画。
