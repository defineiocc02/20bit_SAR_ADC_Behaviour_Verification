...
覆盖：
1. **整定行为**：阈值逼零积分使周期末误差几何收敛；整定周期数由
   [假设] 的阈值增益决定（g=0.9 -> 6，与 [14] 披露"5 或 6"量级一致
   ——是本参数组的演示结果，不是独立验证）；
2. **电荷核算**：S1 闭合时外部参考只需补 C·E，且随整定衰减
   （专利的低功耗论据）；
3. **A06 结构**：粗判决位试承载大参考误差（容忍窗 = 主配置残差
   量程契约 4.6875 mV），低位试的参考误差被压到 20b 级——机制层
   解释，不含 RA/ADC2 联合建立；
4. **精度换算**与入口拒绝。
"""
...
class TestPrecisionConversion:
    def test_reference_precision_bits_identity(self):
        """σ = Δ/√12、Δ = span/2^b 的反解：N = log2(span/(√12·σ))。

        第八份外部复核订正：旧版多一个因子 2（少算 1 bit）。
        """
        # span=3 V、20b -> σ ≈ 41.2 µV
        sigma_20b = 3.0 / (np.sqrt(12.0) * 2.0**20)
        ...
        # 复核者的直接例子：span 3 V、rms 1 µV -> 19.724 bit（非 18.724）
        assert reference_precision_bits(1e-6, 3.0) == pytest.approx(19.724, abs=1e-3)
...
class TestGainSensitivity:
    def test_settle_cycle_depends_on_assumed_gain(self):
        """整定数由 [假设] 的阈值增益决定：g=0.9->6、0.6->12、0.3->25（从零计数）。

        这组数钉住"整定 6 周期是本参数组（g=0.9）的演示结果"——
        不是对 [14] '5 or 6' 披露的独立验证（第八份外部复核）。
        """
        for g, expected in ((0.9, 6), (0.6, 12), (0.3, 25)):
            r = RefTrackSim(RefTrackConfig(threshold_gain=g)).run()
            assert r.settle_cycle == expected, f"g={g}"

    def test_a06_window_uses_main_config_contract(self, run):
        """容忍窗 = 主配置残差量程契约（ADC2 余量/G0 = 4.6875 mV），非 Δ1/2。"""
        cfg = RefTrackConfig()
        assert cfg.tol_first_v == pytest.approx(0.15 / 32.0)
        assert run.a06["tolerance_first_trial_v"] == pytest.approx(cfg.tol_first_v)
        assert "非 Δ1/2 口径" in run.a06["note"]
