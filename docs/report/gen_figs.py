...
C_BLUE = "#2C6E9B"; C_ORANGE = "#E58234"; C_GREEN = "#4C9F70"
C_RED = "#C0504D"; C_GRAY = "#8A8A8A"; C_PURPLE = "#7B68AE"
plt.rcParams.update({
...
def left_right(w=6.6, h=2.75, ratios=(1, 1.25)):
    fig, (axL, axR) = plt.subplots(
        1, 2, figsize=(w, h), gridspec_kw={"width_ratios": ratios})
    for ax in (axL, axR):
        ax.grid(False)
...
def tag(ax, text, xy=(0.02, 0.97), color="#333", weight="bold", size=8.5):
    ax.text(xy[0], xy[1], text, transform=ax.transAxes, va="top",
...
fin = 0.18
def peak(x, x0, w, a): return a * np.exp(-((x - x0) / w) ** 2)
...
for x0, lab, col in [(fin, "$f_{IN}$", C_BLUE), (0.5 - fin, "$f_S/2{-}f_{IN}$", C_RED),
                     (0.5 + fin, "$f_S/2{+}f_{IN}$", C_RED)]:
    axL.annotate(lab, xy=(x0, peak(np.array([x0]), x0, 0.006, 1.0)[0]),
                 xytext=(x0, 1.12), ha="center", fontsize=7.5,
                 color=col, fontweight="bold")
axL.axvline(0.5, color=C_GRAY, lw=0.7, ls="--")
axL.text(0.5, -0.16, "$f_S/2$", ha="center", fontsize=7.5, color=C_GRAY)
axL.set_ylim(-0.22, 1.25); axL.set_xlim(0, 1.0)
tag(axL, "披露机制 [00]\n交织杂散：位置 $f_S/2\\pm f_{IN}$（结构性）\nspare/洗牌打散杂散")
axL.annotate("", xy=(0.5 - fin, 0.34), xytext=(0.5 - fin, 0.02),
...
for b, v in zip(bars, spurs):
    axR.text(b.get_x() + b.get_width() / 2, v - 1.8, f"{v:.1f}", ha="center",
             va="top", fontsize=7, color="#333", fontweight="bold")
axR.set_xticks(range(5)); axR.set_xticklabels(labels, fontsize=6.8)
axR.set_ylabel("$f_S/2{-}f_{IN}$ 杂散 (dBc)")
axR.set_ylim(-80, 4)
axR.grid(axis="y", alpha=0.25)
gain_db = spurs[1] - spurs[2]  # spread=0.3: 固定组 - 洗牌
axR.annotate("", xy=(2, spurs[2] - 1.5), xytext=(1, spurs[1] - 1.0),
             arrowprops=dict(arrowstyle="->", color=C_ORANGE, lw=1.4))
axR.text(1.44, (spurs[1] + spurs[2]) / 2 - 4, f"洗牌抑制 {gain_db:.1f} dB", fontsize=7.5,
         color=C_ORANGE, fontweight="bold", rotation=90)
axR.text(0.02, 0.03, "模型: pipeline.interleave｜洗牌=非因果实现（限定）",
         transform=axR.transAxes, fontsize=6.2, color=C_GRAY)
...
axL.set_ylim(-0.6, 3.4); axL.set_xlim(0, 1)
axL.set_xlabel("归一化输入码"); axL.set_ylabel("INL (LSB)")
axL.set_xticks([0, 0.5, 1.0]); axL.set_yticks([0, 1, 2, 3])
axL.grid(alpha=0.2)
tag(axL, "本模型误差形状示意（自绘）\n非原文实测曲线；论文披露值：\nINL ≈ 2.2 LSB（量级）", xy=(0.30, 0.97))
...
sw = D["s13"]["ron_sweep"]
rho = [r["rho"] for r in sw]; inlmax = [r["INL_max_LSB20"] for r in sw]
...
_rho_t = D["s13"]["rho_max_for_2p2LSB"]; _inl_t = D["s13"]["rho_max_INL_at_rho_max"]
axR1.plot([_rho_t], [_inl_t], "v", color=C_ORANGE, ms=7, zorder=5)
axR1.annotate(f"ρ={_rho_t} → {_inl_t:.2f} LSB（条件性）", xy=(_rho_t, _inl_t), xytext=(0.09, 4.5),
              fontsize=7, color=C_ORANGE, fontweight="bold",
              arrowprops=dict(arrowstyle="->", color=C_ORANGE, lw=0.9))
axR1.set_xlabel("Ron 码调制系数 ρ"); axR1.set_ylabel("INL$_{max}$ (LSB)")
axR1.set_xlim(-0.012, 0.32); axR1.set_ylim(-2, 48)
axR1.legend(loc="upper left", frameon=False)
axR1.grid(alpha=0.25)
tag(axR1, "M3｜ρ 扫描：条件性吻合", xy=(0.42, 0.97), size=7)

rows = D["s13"]["rows"]
labs = [r["设置"].replace("输入建立+Ron码调制", "建立+Ron调制").replace("数字串扰(共模+单位)", "数字串扰") for r in rows]
...
axR2.set_yticks(range(len(rows))); axR2.set_yticklabels(labs, fontsize=6.2)
...
for x0, col in [(0.16, C_GRAY), (0.36, C_BLUE)]:
    axL.annotate("", xy=(x0, 0.42), xytext=(x0, 0.14),
                 arrowprops=dict(arrowstyle="-", color=col, lw=1.2))
    axL.plot([x0 - 0.012, x0 + 0.012], [0.14, 0.14], color=col, lw=1.6)
axL.annotate("", xy=(0.36, 0.30), xytext=(0.16, 0.30),
             arrowprops=dict(arrowstyle="->", color=C_ORANGE, lw=1.2))
axL.text(0.26, 0.335, "粗码误差 → 残差移位 ×G₀ 放大", ha="center", fontsize=6.6, color=C_ORANGE)
axL.text(0.26, 0.05, "窗口内：输出不受影响（自愈）\n越界：误差直接泄漏到输出", ha="center", fontsize=7.2, color="#333")
axL.set_xlim(0, 0.55); axL.set_ylim(0, 1.0)
tag(axL, "披露机制 [09] §1.3\n量化器分离 + 误差自愈窗口")
...
axR1.text(w_["predicted_window_mV"] - 0.4, 2.5, "预测 4.6875 mV", rotation=90,
          fontsize=6.5, color=C_GREEN, ha="right", fontweight="bold")
axR1.text(w_["measured_window_mV"] + 0.4, 2.5, "实测 4.4531 mV", rotation=90,
          fontsize=6.5, color=C_ORANGE, fontweight="bold")
axR1.set_xlabel("注入粗码误差 (mV)"); axR1.set_ylabel("输出误差 RMS (µV)")
axR1.set_ylim(0.5, 6000); axR1.grid(alpha=0.25, which="both")
tag(axR1, "Q7｜窗口扫描：越界即泄漏（对数轴）", xy=(0.30, 0.97), size=7)

cc = D["s9"]["coarse_correction"]
dt = [r["dtau_ps"] for r in cc]; sn = [r["SNDR_dB"] for r in cc]
...
for x_, y_ in zip(dt, sn):
    axR2.text(x_ + 6, y_ + 1.2, f"{y_:.1f}", fontsize=6.5, color=C_PURPLE)
axR2.set_xlabel("Δτ (ps)"); axR2.set_ylabel("SNDR (dB)")
axR2.set_ylim(55, 100); axR2.grid(alpha=0.25)
...
for i, (t, sub, col) in enumerate(items):
    ...
    if i < 2:
        axL.annotate("", xy=(0.26, y - 0.115), xytext=(0.26, y - 0.18),
                     arrowprops=dict(arrowstyle="->", color=C_GRAY, lw=0.9))
axL.text(0.26, 0.005, "三要素严格配对：α=(N−2D)/N 恒定衰减；增强 2 bit",
...
axR1.set_ylim(93.2, 93.9); axR1.set_ylabel("SNDR (dB)")
axR1.set_xticks(xb); axR1.set_xticklabels(mlabels, fontsize=6.8)
axR1.grid(axis="y", alpha=0.25)
axr2 = axR1.twinx(); axr2.grid(False)
...
axr2.set_ylim(0, 5.5)
for x_, v in zip(xb + 0.18, ovf):
    axr2.text(x_, v + 0.12, f"{v:.1f}%", ha="center", fontsize=6.4, color=C_ORANGE)
axr2.spines["right"].set_visible(True); axr2.spines["right"].set_color(C_ORANGE)
tag(axR1, "M8｜等代价对比：采样态不占输入量程", xy=(0.02, 0.97), size=7)
axR1.text(0.03, 0.78, "α(配置)−α(掩码)=1.1e-16（Q9 闭合）\n代价=−20·log₁₀(α)=0.067 dB",
          transform=axR1.transAxes, fontsize=6.4, color="#333")
...
for i in range(n):
    ...
    axL.add_patch(Rectangle((0.03 + i * (0.94 / n), 0.55), 0.94 / n * 0.82, 0.14, fc=col, ec="white", lw=0.4))
axL.text(0.5, 0.78, "任意置换 state：选中单位数 = 逻辑码（k=10）", ha="center", fontsize=7.2, color=C_BLUE, fontweight="bold")
axL.text(0.5, 0.40, "✓ 名义总电荷守恒（结构性质）\n✗ 桥接 C_C 比例误差锯齿：置换无效\n✗ 增益/共模/边界误差：不可修复",
         ha="center", fontsize=7.0, color="#333", linespacing=1.6)
axL.text(0.5, 0.10, "排列空间：64 个联合排列（R16：512 状态标签坍缩）", ha="center", fontsize=7.0, color=C_ORANGE, fontweight="bold")
axL.set_xlim(0, 1); axL.set_ylim(0, 1.0)
...
for i, (b, v, g) in enumerate(zip(bars, ranks, gains)):
    axR1.text(b.get_x() + b.get_width() / 2, v + 10, f"rank={v}", ha="center", fontsize=7.2, fontweight="bold", color="#333")
    axR1.text(b.get_x() + b.get_width() / 2, max(v + 52, 90), f"校准改善\n{g:.1f}×", ha="center", fontsize=6.5,
              color=C_GREEN if g >= 3 else C_RED, fontweight="bold")
axR1.set_xticks(range(3)); axR1.set_xticklabels(mlabels5, fontsize=6.5)
axR1.set_ylabel("rank(U)（512 维校准空间）")
axR1.set_ylim(0, 640); axR1.grid(axis="y", alpha=0.25)
...
a_vec = np.array([1.0, 0.0]); b_vec = np.array([0.85, 0.45])
k = 0.98
res = a_vec - k * b_vec
for vec, col, lab in [(a_vec, C_BLUE, "采样噪声 a"),
                       (k * b_vec, C_ORANGE, "κ·相关路径 b"),
                       (res, C_GREEN, "残余 a−κb")]:
    axL.annotate("", xy=(vec[0] * 0.8 + 0.5, vec[1] * 0.8 + 0.5), xytext=(0.5, 0.5),
                 arrowprops=dict(arrowstyle="->", color=col, lw=1.8))
    axL.text(vec[0] * 0.88 + 0.5, vec[1] * 0.88 + 0.52, lab, fontsize=7.2, color=col, fontweight="bold")
axL.add_patch(Rectangle((0.03, 0.03), 0.94, 0.94, fill=False, ec="none"))
axL.text(0.5, 0.02, "κ_opt = aᵀΣb / (bᵀΣb + σ²_eN)（联合最优，R18）", ha="center", fontsize=7.2, color="#333", fontweight="bold")
axL.set_xlim(0.25, 1.80); axL.set_ylim(-0.1, 1.1)
...
for i, (a_, m_) in enumerate(zip(sa, sm)):
...
axR1.set_xticks(xb); axR1.set_xticklabels([f"γ={g}" for g in gam])
axR1.set_ylabel("σ_res (µV)"); axR1.set_ylim(0, 1.75)
axR1.legend(frameon=False, loc="upper left")
axR1.grid(axis="y", alpha=0.25)
_dev = max(abs(m - a) / a for a, m in zip(sa, sm)) * 100
axR1.text(0.98, 0.97, f"X1：最大偏差 {_dev:.2f}%", transform=axR1.transAxes, ha="right", va="top", fontsize=7, fontweight="bold", color="#333")
axR1.text(0.03, 0.80, "X2：开 KTC → 95.00 dB\n（理想读出上界，s6）",
          transform=axR1.transAxes, fontsize=6.4, color=C_GREEN, fontweight="bold")
...
axL.text(0.5, 0.14, "白噪声 ×10^(1.6/20)（折叠）\n1/f 整体移除（结构）", ha="center", fontsize=7.0, color="#333")
axL.set_xlim(0, 1); axL.set_ylim(0, 1.0)
tag(axL, "披露 [00_1] p.34–35\nAZ 代价 −1.6 dB；ADC2 动态带宽 +1.3 dB\n（本模型=效果预算，非相位级）")
...
for i, (p_, m_) in enumerate(zip(pred, meas)):
    axR.text(i - 0.19, p_ - 0.09 if p_ < 0 else p_ + 0.04, f"{p_:+.3f}", ha="center", fontsize=6.3, color=C_BLUE)
    axR.text(i + 0.19, m_ - 0.09 if m_ < 0 else m_ + 0.04, f"{m_:+.3f}", ha="center", fontsize=6.3, color=C_ORANGE)
axR.set_xticks(xb); axR.set_xticklabels([g[1] for g in groups])
axR.set_ylabel("SNDR 变化 (dB)")
axR.axhline(0, color="#333", lw=0.7)
axR.set_ylim(-1.9, 1.75); axR.grid(axis="y", alpha=0.25)
axR.legend(frameon=False, loc="upper left")
tag(axR, "Q8｜预测↔实测最大偏差 0.006 dB", xy=(0.55, 0.97), size=7)
axR.text(0.99, 0.03, "与披露差额 = 参照系差异（不硬凑）", transform=axR.transAxes,
         ha="right", fontsize=6.2, color=C_GRAY)
...
axL.text(need / 2, 0.66, "披露匹配目标\nσ ≤ 300 ppm\n[00_1]", ha="center", fontsize=7.2, color=C_GREEN, fontweight="bold")
axL.text(720, 0.66, "PDK 估算 1117 ppm\n（3.7× 于披露目标）", ha="center", fontsize=7.2, color=C_RED, fontweight="bold")
axL.annotate("", xy=(need, 0.42), xytext=(1117, 0.42),
             arrowprops=dict(arrowstyle="<->", color=C_ORANGE, lw=1.3))
axL.text((need + 1117) / 2, 0.48, "3.7×", ha="center", fontsize=8.5, color=C_ORANGE, fontweight="bold")
axL.text(650, 0.24, "等面积电容补齐需 3.7² ≈ 14× 面积；\n且面积改变会连带采样噪声/负载/建立\n→ 单纯扩面积非等效替代",
         ha="center", fontsize=6.6, color="#333")
axL.text(650, 0.05, f"模型均值口径 SNDR≥93 dB 达标上限 ≈{mean_lim:.0f} ppm（budget.rows）",
         ha="center", fontsize=6.6, color=C_PURPLE, fontweight="bold")
axL.set_xlim(0, 1300); axL.set_ylim(0, 1.0)
axL.set_xlabel("电容失配 σ (ppm)")
tag(axL, "披露论证 [00_1]：>12b AC matching → 校准动机\n（模型等效匹配 13.29 bit ≥ 12，Q4；映射链见正文）", xy=(0.50, 0.99), size=6.6)
...
axR1.set_xlabel("失配 σ (ppm)"); axR1.set_ylabel("SNDR (dB)")
axR1.set_xlim(80, 1200); axR1.set_ylim(90.4, 94.0)
...
axR2.text(62, float(np.mean(chips)) + 0.03, f"均值 {np.mean(chips):.2f} dB", fontsize=6.5, color=C_BLUE, va="bottom", ha="left")
axR2.set_xlabel("失配 σ (ppm)"); axR2.set_ylabel("MC 单片 SNDR (dB)")
axR2.set_xlim(60, 140); axR2.set_ylim(93.2, 93.8)
