...
\textbf{10 项机制级对照}、\textbf{2 项原创扩展证据}（KTC 相消，
...
参考）在当前源码中\textbf{未实现}，本报告如实登记为未复现项。
...
机制/结构（定性或条件性） & M1--M10（M3 条件性；M2 带限定；M4 机制对照非严格分解）\\
...
[09]--[14] 的逐专利对齐状态（三态：对齐 / 机制对齐但口径不同 /
未实现），与仓库对比文档 \texttt{engineering\_vs\_patents\_papers.md}
保持一致（该文档已经第六份复核订正：[12] 编号修正为
US~10,707,889~B1；18-slice、共享 RA、AZ 等条目的状态已与实际执行
路径对齐）。
...
{}[12] US 10,707,889 B1 & 交织跟踪状态更新 & 无（源码零命中） & \textcolor{badred}{未实现}\\
{}[13] US 10,541,702 B1 & 辅助输入抵消输入电荷 & 无（源码零命中） & \textcolor{badred}{未实现}\\
{}[14] US 10,826,519 B1 & 低功耗参考 & 无（参考为 A06 缺口） & \textcolor{badred}{未实现}\\
\bottomrule
\end{tabular}
\caption{专利对应状态（三态）}\label{tab:pat}
\end{table}
...
\begin{enumerate}[itemsep=2pt]
  ...
  \item \textbf{未复现}：[12]/[13]/[14] 机制零实现；论文三维 DEM 的
        定量收益（排列空间 64$\neq$512）；实测硅片行为超出口径。
