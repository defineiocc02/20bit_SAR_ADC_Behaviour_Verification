...
每个物理单元的系数 `W[s][u]` 是有效 C/Cf（子单元含桥接系数β）的正 Q30，
已包含电荷增益，不能再无条件乘/除32。定义：

- `T = ΣW`：本次参与 RDAC 的所有物理单元权重。
- `G = Σ(W*a)`：实际采集输入的单元权重；采样 dither 专用单元 `a=0`。
- `R = ΣW - 2Σ(W*on) + Σ(W*r)`：实际开关轨对应的带符号权重。
- `F/O/I`：后级ADC2电压、同域偏置、独立已知公共注入，均除以输入Vfs后编码为Q32。
  `F` 沿用仓库的RA后ADC2电压域；`I` 对应 `common_injection_v`，作用于全部参与单元。
...
RDAC 译码减去同一笔 dither；独立公共注入 `inj_q` **不再包含该量化器 dither**，否则双重扣除。
