...
**但这些 Δ 一个都不能当作"修复的效果"。** 自助法检验（`significance.json`，图 7，
20 000 次重抽样）把新旧两批逐颗 SNDR 合并成池、各自独立重抽 n 颗，得到 Δ 的零分布
（"纯粹换一批随机数"能造成多大差异）：

| 分区 | n | Δ 最差芯片 | 零分布 95% 带宽 | z |
| :--- | ---: | ---: | ---: | ---: |
| mc | 16 | −0.1580 dB | ±0.2989 | −1.04 |
| mc_cal_off | 60 | −0.1914 dB | ±0.2735 | −1.37 |
| mc_cal_on | 60 | −0.0524 dB | ±0.1197 | −0.86 |
| mc_pdk_off | 60 | −2.1776 dB | ±3.2233 | −1.32 |
| mc_pdk_on | 60 | −1.0014 dB | ±1.3623 | −1.44 |

**10 项检验（5 个分区的 Δmin 与 Δσ）全部落在零分布的 95% 区间内**（|z| ≤ 1.44）。
即：在 n=16 / n=60 下，"最差一颗"与"样本标准差"本身就是高方差统计量，
上述可见变化**与"只是换了一批随机数"无法区分**。

因此本版的诚实结论是：

- **修复的正确性不由这些数字支撑，而由代码级缺陷本身支撑**（同一整数种子播种两条流，
  探针可复现地证实流重合）——这是一个**方法学缺陷的移除**，不需要靠输出数字来"证明"。
- **修复没有可检测地改变任何统计结论**。MC 与良率指标在这些样本量下由抽样噪声主导，
  v8.1.0 与 v8.2.0 的 MC/良率结论**互不矛盾**，既不能据此宣称良率变差，也不能宣称变好。
- **适用域边界**：若要用 MC 指标分辨量级 ~0.2 dB（pdk_off 的 Δmin）的差异，按零分布
  带宽 ~3.2 dB 反推需把芯片数提高约 $(3.2/0.2)^2\approx 250$ 倍；本仓库的 16/60 颗
  系综**没有**这个分辨力。**不要**拿本文的 MC 极值做良率论断。

> **发布后更正（2026-09-25，同一日）**：本段初版写作"旧口径低估了尾部、PDK 良率下界
> 下移 2.18 dB 是更诚实的下界"，并且 `README` 两版与 GitHub Release notes 同此措辞。
> 该措辞**过度解读**——它把一个落在抽样噪声内的差异当成了修复效应，而当时并未做显著
> 性检验。发现后已改为上面的口径，并补做自助法检验（图 7 / `significance.json`）。
> 已发布的 tag `v8.2.0` **不追改**（不移动已发布 tag）；本更正落在后续提交与 Release
> notes 中，差异可见、无静默改写。
...
- `docs/release_v8.2.0/make_delta_charts.py` 与 `docs/release_v8.2.0/fig/`：**7 张**数据对比图
  （逐叶子变更图、关键指标对比、MC 分布箱线、新旧 RNG 散点、22 项发现矩阵、探针效应、
  **自助法显著性检验**）+ `significance.json`（全部 10 项检验的零分布与实测 Δ）。
