...
> **撤回（2026-09-11，第四轮复核）**：本节的两个出口都**没有**被采用，因为第四节
> 的复核指出这条判据**绑定了错误的节点**。旧口径按 ADC2 量程余量算，而 ADR 0006
> 已在数字域扣除校正量，故 2.5465 MHz 是**假失败**；改绑观测通路节点后同一配置
> 得 134.4541 MHz，PASS。`KNOWN_LIMITS` 里的该条目**已删除**，键名也随口径改掉。
> 本节以下内容作为当时的判断记录**保留存档**，不再代表当前状态 —— 现行结论见
> `docs/review_response_2026-09-11c.md` §1 与 `docs/model_scope.md` §6。
